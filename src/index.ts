  ...
  public static init(tsBackTimer: TsBackTimerLibrary) {
    ...
    if (container.getElementsByTagName('button').length > 0)
      return console.error(
        'TsBackTimer already running, please refresh and rerun script!'
      )
    ...
    for (var i = 0; i < trs.length; i++) {
      ...
      const el =
        i === 0 ? tr.getElementsByTagName('th') : tr.getElementsByTagName('td')
      let newNode =
        i === 0 ? document.createElement('th') : document.createElement('td')
      let text =
        i === 0 ? document.createTextNode('') : document.createTextNode('')
      let childNode = el[0]
